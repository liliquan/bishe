            ...
            <div class="ibox-content">
                ...
                <form class="form-horizontal m-t-md" action="{{route('rules.update',$rule->id)}}" method="POST">
                            ...
                            <select name="parent_id" class="form-control">
                                ...
                                @foreach($rules as $k=>$item)
                                    <option value="{{$item['id']}}"
                                            @if($rule->parent_id == $item['id']) selected="selected"@endif >{{$item['_name']}}</option>
                                @endforeach
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-3">
                            <input type="text" name="fonts" id="fonts" onclick="showicon()" value="{{$rule->fonts}}"
                                   placeholder="菜单图标" class="form-control">
                            @if ($errors->has('fonts'))
                                <span class="help-block m-b-none"><i class="fa fa-info-circle"></i>{{$errors->first('fonts')}}</span>
                            @else
                                <span class="help-block m-b-none"><i
                                            class="fa fa-info-circle"></i> 采用Font Awesome字体图标</span></div>
                        @endif
                    </div>
                    <div class="hr-line-dashed m-t-sm m-b-sm"></div>
                    <div class="form-group">
