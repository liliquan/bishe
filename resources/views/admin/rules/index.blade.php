    ...
    <div class="row">
        <div class="col-sm-12">
            ...
            <div class="ibox-content">
                <a href="{{route('rules.create')}}" link-url="javascript:void(0)">
                    <button class="btn btn-primary btn-sm" type="button"><i class="fa fa-plus-circle"></i> 添加权限</button>
                </a>
                    ...
                    @foreach($rules as $k=>$item)
                        <tr>
                            ...
                            <td style="text-align:center"><i
                                        class="fa fa-{{isset($item['fonts']) ? $item['fonts'] : 'desktop'}}"></i></td>
                            ...
                            <td class="text-center">
                                <a href="{{route('rules.edit',$item['id'])}}">
                                    <button class="btn btn-primary btn-xs" type="button"><i class="fa fa-paste"></i> 修改
                                    </button>
                                </a>
                                @if($item['is_hidden'] == 1)
                                    <a href="{{route('rules.status',['status'=>0,$item['id']])}}">
                                        <button class="btn btn-info btn-xs" type="button"><i class="fa fa-warning"></i>
                                            显示
                                        </button>
                                    </a>
                                @else
                                ...
                                <form class="form-common" action="{{route('rules.destroy',$item['id'])}}" method="post">
                                    {{ csrf_field() }}
                                    {{ method_field('DELETE') }}
                                    <button class="btn btn-danger btn-xs" type="submit"><i class="fa fa-trash-o"></i> 删除
                                    </button>
                                </form>
                            </td>
                        </tr>
